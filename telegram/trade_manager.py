...
class TelegramTradeManager:
    ...
    def publish_turtle_soup_scalp_signal(
        self,
        symbol: str,
        signal: TradeSignal,
        *,
        ...
        """Send a VIP 2 Turtle Soup scalp signal to Telegram and store it as active."""
        message_id: int | None = None
        if self.telegram_bot is not None:
            message_id = self.telegram_bot.send_turtle_soup_scalp_trade_signal(
                symbol,
                signal,
                timeframe=timeframe,
            )
